...
describe('AppLogger Test', function () {
  it('Success call all log functions', function () {
    ...
    container
      .bind(Type.ExternalLogger)
      .toFunction((level: Level, message: string, optional?: any) => {
    ...
    expect(() =>
      appLogger.error('Error Message', { trace: 'test' }),
    ).to.not.throw();
    expect(() =>
      appLogger.warn('Warning Message', { trace: 'test' }),
    ).to.not.throw();
    expect(() =>
      appLogger.log('Log Message', { trace: 'test' }),
    ).to.not.throw();
    expect(() =>
      appLogger.verbose('Verbose Message', { trace: 'test' }),
    ).to.not.throw();
    expect(() =>
      appLogger.debug('Debug Message', { trace: 'test' }),
    ).to.not.throw();
    expect(() =>
      appLogger.silly('Silly Message', { trace: 'test' }),
    ).to.not.throw();
  });
